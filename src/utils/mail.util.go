...
func SendSignUpSecretCode(to string, cc *string, account models.AccountCreatable) error {
	...
	body := fmt.Sprintf(`
		...
		Your Profile Information here:
			- Username: %s
			- Secret Code: %d
		Please keep the secret code carefully, you must use this secret code to publish an order!
...
func SendResetPassword(to string, cc *string, account models.AccountCreatable) error {
	host := "smtp.gmail.com"
	port := 587
	username := os.Getenv("GMAIL_USERNAME")
	password := os.Getenv("GMAIL_APPLICATION_PASSWORD")
	from := os.Getenv("HOST_GMAIL_ADDRESS")
	sub := "NOTIFICATION RESET PASSWORD EMAIL - WELCOME TO OUR SERVICE"
	body := fmt.Sprintf(`
		Welcome: %s to our service:
